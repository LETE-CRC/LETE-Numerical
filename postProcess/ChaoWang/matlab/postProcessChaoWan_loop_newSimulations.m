...
%     filename = sprintf('/media/sacomano/ac6183b7-5ff6-4f64-995a-6c67c36d7b98/Simulations/Converge/noCOVID19/ValidacaoChao/parcel_data/validationch00000%d.par',j);
    filename = sprintf('/media/sacomano/ac6183b7-5ff6-4f64-995a-6c67c36d7b98/Simulations/Converge/noCOVID19/ValidacaoChao/VALIDATION_Wang_v06/par/validation00000%d.par',j);
    
    data_crude = importParData2(filename, startRow, endRow);
    data_crude(:,6) = data_crude(:,6)*2*1e6; % transformation from radius to diameter
    ...
    for i = 1:size(origBands,1)
%         b              = find(data_crude(:,6)>=diam_min(i,1) & data_crude(:,6)<diam_max(i,1));
        b              = find(data_crude(:,6)>=diam_min(i,j) & data_crude(:,6)<diam_max(i,j));
        bins(i)        = size(b,1);
        ...
        data_band(j,i,6) = mean(data_crude(b,1).*data_crude(b,1) + data_crude(b,3).*data_crude(b,3));
    end
